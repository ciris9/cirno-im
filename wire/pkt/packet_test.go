...
func TestReadPkt(t *testing.T) {
	...
	packet := New("auth.login.aa", WithSequence(seq), WithStatus(Status_Success))
	assert.Equal(t, "auth", packet.ServiceName())
	// assert.Equal(t, "login.aa", packet.CommandPath())

	packet = New("auth.login", WithSequence(seq), WithStatus(Status_Success))
	packet.WriteBody(&LoginRequest{
		Token: "test token",
	})
	packet.AddMeta(&Meta{
		Key:   "test",
