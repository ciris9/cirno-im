...
import (
	...
	"cirno-im/naming"
	"cirno-im/services/router/conf"
	"cirno-im/services/router/ipregion"
	"cirno-im/wire"
...
func (r *RouterApi) Lookup(c iris.Context) {
	...
	for i, h := range hits {
		domains[i] = h.GetMetadata()["domain"]
	}
...
func selectGateways(token string, gateways []cim.ServiceRegistration, num int) []cim.ServiceRegistration {
	if len(gateways) <= num {
		return gateways
	}
	...
	i := slots[slot]
	res := make([]cim.ServiceRegistration, 0, num)
	for len(res) < num {
