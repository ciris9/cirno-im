...
![6298df7752a1976beb197869ca2a293e](./6298df7752a1976beb197869ca2a293e.jpg)

gateway与server层之间，会存在大量的消息，而且是长连接的，必然会导致大量的开销。最终甚至可能会导致server宕机，超高并发下，无法保证服务高可用。

可以选择在gateway和server层之间，加一个消息队列集群进行异步消息，当然发送的消息还是自定义的logic packet。

另外，这里的离线消息，原来是从关系型数据库内拉取的，效率不高，考虑到离线数据拉取的消息量有一定的限制，可以现将其存到redis集群内，作为一个缓存，可以加快离线同步的操作。

对于关系型数据库的选型，传统的mysql由于历史原因，实际上是趋于单机数据库的。如果我们想要在大数据量的场景下，需要选择真正意义上的分布式数据库，如tidb或者dorisdb
...
自定义logic packet和basic packet，分别用于用户服务间通信以及心跳。
